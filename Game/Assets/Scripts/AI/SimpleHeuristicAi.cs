...
public static class SimpleHeuristicAi
{
    public static bool TryActOnce(UnitActor unit)
    {
        ...
        if (TryUseRedBull(unit))
            return true;

        return false;
    }

    private static bool TryUseRedBull(UnitActor unit)
    {
        if (unit.Inventory == null || unit.Inventory.CountOf(ItemKind.RedBull) <= 0)
            return false;
        // 已用掉移动或普攻时更值得刷新；纯开局不喝
        if (!TurnManager.Instance.HasMoved && !TurnManager.Instance.HasMeleeAttacked)
            return false;
    ...
    private static bool TryUseMilk(UnitActor unit)
    {
        if (unit.Inventory == null || unit.Inventory.CountOf(ItemKind.Milk) <= 0)
            return false;
    ...
    private static bool TryUseWeatherBullet(UnitActor unit)
    {
        if (unit.Inventory == null)
            return false;

        bool hasRaincoat = ItemInfo.HasEquippedRubberRaincoat(unit);
        ItemKind prefer;
        bool hasPrefer = false;

        if (hasRaincoat && WeatherService.Current != WeatherType.Rain
            && unit.Inventory.CountOf(ItemKind.WeatherRain) > 0)
        {
            prefer = ItemKind.WeatherRain;
            hasPrefer = true;
        }
        else if (!hasRaincoat && WeatherService.Current == WeatherType.Rain
                 && unit.Inventory.CountOf(ItemKind.WeatherClear) > 0)
        {
            prefer = ItemKind.WeatherClear;
            hasPrefer = true;
        }
        else if (WeatherService.Current != WeatherType.Fog
                 && unit.Inventory.CountOf(ItemKind.WeatherFog) > 0
                 && unit.Hp * 10 < unit.MaxHp * 5)
        {
            prefer = ItemKind.WeatherFog;
            hasPrefer = true;
        }
        else
        {
        ...
        if (!hasPrefer)
            return false;
        return ActionService.TryUseWeatherBullet(unit, prefer);
    }
    ...
    private static bool TryEquipGear(UnitActor unit)
    {
        ...
        bool wearingAccessory = false;
        bool wearingVehicle = false;
        for (int i = 0; i < items.Count; i++)
        {
            ...
            if (items[i].Kind == ItemKind.EnergyShield)
                wearingShield = true;
            if (ItemInfo.GetEquipSlot(items[i].Kind) == ItemInfo.EquipSlot.Accessory)
                wearingAccessory = true;
            if (ItemInfo.GetEquipSlot(items[i].Kind) == ItemInfo.EquipSlot.Vehicle)
                wearingVehicle = true;
        }
        ...
        {
            int bestMelee = -1;
            int bestMeleeScore = -1;
            for (int i = 0; i < items.Count; i++)
            {
                if (items[i].Equipped || !ItemInfo.IsMeleeWeapon(items[i].Kind))
                    continue;
                int score = items[i].Kind == ItemKind.Longsword ? 3
                    : items[i].Kind == ItemKind.ArmorPiercingBlade ? 2
                    : items[i].Kind == ItemKind.CursedBlade ? 1
                    : 1;
            ...
            if (bestMelee >= 0 && ActionService.TryToggleEquip(unit, bestMelee))
                return true;
        }
        ...
        for (int i = 0; i < items.Count; i++)
        {
            ...
            {
                int score = items[i].Kind == ItemKind.IronArmor ? 3
                    : items[i].Kind == ItemKind.RubberRaincoat
                        ? (WeatherService.Current == WeatherType.Rain ? 3 : 2)
                        : items[i].Kind == ItemKind.ThornsArmor ? 2
                        : items[i].Kind == ItemKind.TacticalVest ? 2
                        : 1;
        ...
        if (!wearingAccessory)
        {
            ...
            {
                if (items[i].Equipped) continue;
                if (items[i].Kind == ItemKind.Amulet && ActionService.TryToggleEquip(unit, i))
                    return true;
            }
        }

        if (!wearingVehicle && unit.CurrentTile == TileType.Ice)
        {
            for (int i = 0; i < items.Count; i++)
            {
                if (items[i].Equipped) continue;
                if (items[i].Kind == ItemKind.IceSkates && ActionService.TryToggleEquip(unit, i))
                    return true;
            }
        }
    ...
    private static bool TryMeleeBest(UnitActor unit)
    {
        ...
        bool cursed = ItemInfo.HasEquippedCursedBlade(unit);
        if (cursed && TurnManager.Instance.HasMoved)
            return false;
        ...
        foreach (var other in GameManager.Instance.Units)
        {
            ...
            if (cursed)
            {
                expected = cursedTargetDmg;
                score = expected * 10f - dist - cursedSelfDmg * 8f;
                if (cursedSelfDmg >= unit.Hp && expected < other.Hp)
                    continue; // 自杀却杀不掉对方则跳过
            }
            else
            {
    ...
    private static bool TryBoomerangBest(UnitActor unit)
    {
        int idx = unit.Inventory.Items.FindIndex(i => i.Kind == ItemKind.Boomerang);
        if (idx < 0)
            return false;
        ...
        foreach (var other in GameManager.Instance.Units)
        {
            ...
            if (!StealthService.CanTargetDespiteHidden(unit, other))
                continue;
            if (!VisibilityService.CanSeeCell(unit, other.Cell))
                continue;
            int dist = grid.GetManhattanDistance(unit.Cell, other.Cell);
            if (dist > ItemInfo.BoomerangRange)
                continue;

            int expected = Mathf.Max(0, ItemInfo.BoomerangDamage - other.CurrentDef);
            float score = expected * 10f - dist;
            if (other.IsDying) score += 120f; // 击杀可回手
            else if (expected >= other.Hp) score += 40f;
            if (score > bestScore)
            {
                bestScore = score;
                best = other;
        ...
        if (best == null || bestScore < 8f)
            return false;

        TurnManager.Instance.EnterShootTargeting(idx, ItemKind.Arrow);
        return ActionService.TryThrowBoomerang(unit, best);
    }
    ...
    private static bool TryMolotovBest(UnitActor unit)
    {
        if (!ItemInfo.CanThrowMolotov(unit))
            return false;
        int gasIndex = unit.Inventory.Items.FindIndex(i => i.Kind == ItemKind.GasolineBottle);
        if (gasIndex < 0)
            return false;
        ...
        for (int dx = -5; dx <= 5; dx++)
        {
            for (int dy = -5; dy <= 5; dy++)
            {
                ...
                float score = 0f;
                if (grid.GetManhattanDistance(cell, unit.Cell) <= blast)
                    score -= damage * 8f;
                ...
                foreach (var other in GameManager.Instance.Units)
                {
                    ...
                    if (grid.GetManhattanDistance(cell, other.Cell) > blast)
                        continue;
                    int expected = Mathf.Max(0, damage); // 法伤不减防
                    score += expected * 12f;
                    if (expected >= other.Hp || other.IsDying)
                        score += 50f;
                    score += 15f; // 着火/火焰区价值
        ...
        TurnManager.Instance.EnterBombTargeting(gasIndex);
        return ActionService.TryThrowMolotov(unit, bestCell);
    }

    private static bool TryPickup(UnitActor unit)
    {
        // 已满或已超重：不再拾取，避免扫光掉落物后卡死无法结束行动
