...
public class PlayerInputController : MonoBehaviour
{
    ...
    private void HandleCellClick(Vector2Int cell)
    {
        ...
        switch (turn.Phase)
        {
            ...
            case TurnPhase.SelectingBombTarget:
            {
                ...
                ItemKind aimKind = ItemKind.Bomb;
                if (pidx >= 0 && pidx < unit.Inventory.Count)
                    aimKind = unit.Inventory.Items[pidx].Kind;
                if (aimKind == ItemKind.Flashbang)
                {
            ...
            case TurnPhase.SelectingShootTarget:
            {
                ...
                int pIdx = turn.PendingItemIndex;
                if (pIdx >= 0 && pIdx < unit.Inventory.Count
                    && unit.Inventory.Items[pIdx].Kind == ItemKind.Boomerang)
                {
                    if (!ActionService.TryThrowBoomerang(unit, target))
                        turn.LogFor(unit, "回旋镖投掷失败（超距、不可见或目标无效）");
                }
                else if (!ActionService.TryShoot(unit, pIdx, target))
    ...
    public void StartMolotovMode(int itemIndex)
    {
        ...
        if (unit == null || unit.IsDying)
            return;
        if (itemIndex < 0 || itemIndex >= unit.Inventory.Count
            || !ItemInfo.IsMolotovKit(unit.Inventory.Items[itemIndex].Kind))
            return;
        if (!ItemInfo.CanThrowMolotov(unit))
        {
            turn.LogFor(unit, "需同时持有【打火机】与【汽油瓶】");
            return;
        }
        turn.EnterBombTargeting(itemIndex);
    ...
    public void StartFlameMode(int itemIndex)
    {
        ...
        {
            turn.LogFor(unit, "火焰喷射器需要【汽油瓶】作为弹药，无法单独喷射");
            return;
        }
    ...
    public void StartBoomerangMode(int itemIndex)
    {
        var turn = TurnManager.Instance;
        var unit = turn?.CurrentUnit;
        if (unit == null || unit.IsDying || itemIndex < 0 || itemIndex >= unit.Inventory.Count)
            return;
        if (unit.Inventory.Items[itemIndex].Kind != ItemKind.Boomerang)
            return;
        ...
        switch (turn.Phase)
        {
            ...
            case TurnPhase.SelectingShootTarget:
            {
                if (turn.PendingItemIndex >= 0 && turn.PendingItemIndex < unit.Inventory.Count)
                {
                    ...
                    int range = w == ItemKind.Boomerang
                        ? ItemInfo.BoomerangRange
                        : unit.GetShootRange(w);
                    MapVisual.Instance?.ShowShootHints(unit, range);
                }
                return;
            }
